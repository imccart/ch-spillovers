...
## Description:   Create risk adjusted price at hospital level for outpatient
##                procedures only


# Risk adjustment ---------------------------------------------------------
price.reg.op <- ch.data.final %>% 
...
for (t in 2010:2015) {
  ...
  pred.fe <- pred.fe %>%
    mutate(Year=t) %>%
    dplyr::select(adj_price_op=effect, aha_hnpi=idx, Year) %>%
  ...
  pred.fe.comp <- pred.fe.comp %>%
    mutate(Year=t) %>%
    dplyr::select(adj_comp_op=effect, aha_hnpi=idx, Year) %>%
    mutate(aha_hnpi=as.character(aha_hnpi))
  ...
  pred.fe.readmit <- pred.fe.readmit %>%
    mutate(Year=t) %>%
    dplyr::select(adj_readmit_op=effect, aha_hnpi=idx, Year) %>%
    mutate(aha_hnpi=as.character(aha_hnpi))
...
hosp.avgs <- claims.only %>%
  ...
  summarize(hcci_price_op=mean(hcci_price),
            Comp_Any_OP=mean(Comp_Any),
            any_readmit_30_op=mean(any_readmit_30),
            any_readmit_60_op=mean(any_readmit_60),
            any_readmit_90_op=mean(any_readmit_90),
            tot_patients_op=n())
