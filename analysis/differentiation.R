...
## Description:   Analysis of CH entry/exit
...
market.tiers.base <- hosp.only %>% 
  select(aha_hnpi, Year, CH_TierAll, zip=hosp_zip) %>%
  mutate(hosp_a=ifelse(CH_TierAll=="Tier A",1,0),
  ...
  summarize(tier_a_hosp=sum(hosp_a, na.rm=TRUE),
            tier_b_hosp=sum(hosp_b, na.rm=TRUE),
            tier_c_hosp=sum(hosp_c, na.rm=TRUE),
            tier_d_hosp=sum(hosp_d, na.rm=TRUE),
            tier_other_hosp=sum(hosp_other, na.rm=TRUE)) %>%
  mutate(tier_ch_hosp=tier_a_hosp+tier_b_hosp,
         tier_nch_hosp=tier_c_hosp+tier_d_hosp+tier_other_hosp) %>%
  group_by(market) %>%
  arrange(market, Year) %>%
  mutate(tier_ch_lag=lag(tier_ch_hosp),
         tier_a_lag=lag(tier_a_hosp),
         tier_b_lag=lag(tier_b_hosp),
         tier_nch_lag=lag(tier_nch_hosp)) %>%
  mutate(ch_entry=ifelse(tier_ch_hosp>tier_ch_lag, 1, 0),
         tiera_entry=ifelse(tier_a_hosp>tier_a_lag, 1, 0),
         tierb_entry=ifelse(tier_b_hosp>tier_b_lag, 1, 0),
         nch_entry=ifelse(tier_nch_hosp>tier_nch_lag, 1, 0))
...
panel.ch <- panelview(mean_price~ch_treat, data=panel.mkt.ch %>% select(mean_price, ch_treat, mkt_num, Year), 
...
ggsave("results/f4-panelview-ch-entry.png", panel.ch)
...
panel.nch <- panelview(mean_price~nch_treat, data=panel.mkt.nch %>% select(mean_price, nch_treat, mkt_num, Year),
          index=c("mkt_num","Year"), legendOff=TRUE, 
          theme.bw=TRUE, by.timing=TRUE, xlab="Year", ylab="Market",
          main="", color=c("white","gray"), axis.lab="time")
ggsave("results/f4-panelview-nch-entry.png", panel.nch)
...
claim.reg.ch <- ch.data.final %>%
  ...
  mutate(log_price=log(hcci_price),
         surg_num=unclass(as.factor(Surgery)),
         type_num=unclass(as.factor(Type)),
         ch=ifelse(CH_TierAll %in% c("Tier A", "Tier B"), 1, 0)) %>%
  group_by(market) %>%
  mutate(max_treat=max(ch_treat, na.rm=TRUE)) %>%
  ungroup()
...
plot.ch <- dwplot(est.ch,
  ...
  scale_y_discrete(labels=c("time_p3"="t+3",
                           ...
                           "time_m2" = "t-2",
                           "time_m3" = "t-3"))
ggsave("results/f5-event-ch.png", plot.ch)
...
## baseline results without covariate adjustments
...
plot.es <- dwplot(event.study,
  ...
  scale_y_discrete(labels=c("-3"="t-3",
                            ...
                            "2" = "t+2",
                            "3" = "t+3")) +
  annotate(geom="text", y=5.5, x=0.20, label=paste0("Overall ATT: [",low.ci.all,", ",high.ci.all,"]"), hjust=0) +
  annotate(geom="text", y=5.5, x=0.18, label=paste0("CH ATT: [",low.ci1,", ",high.ci1,"]"), hjust=0) +
  annotate(geom="text", y=5.5, x=0.16, label=paste0("NCH ATT: [",low.ci2,", ",high.ci2,"]"), hjust=0)
ggsave("results/f6-did-base-ch.png", plot.es)
...
ggsave("results/f6-did-event-ch-all.png", dd.ch.plot)  
...
dd.ch.plot <- ggdid(event.study1) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.12, label=paste0("Overall ATT 95% CI \n[",low.ci1,", ",high.ci1,"]"))
ggsave("results/f6-did-event-ch1.png", dd.ch.plot)  
...
dd.ch.plot <- ggdid(event.study2) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.12, label=paste0("Overall ATT 95% CI \n[",low.ci2,", ",high.ci2,"]"))
ggsave("results/f6-did-event-ch2.png", dd.ch.plot)  
...
ggsave("results/f6-did-event-ch.png", plot.es)




## with residuals (only adjusting for place and procedure)
...
dd.ch.plot <- ggdid(event.study.all) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.12, label=paste0("Overall ATT 95% CI \n[",low.ci.all,", ",high.ci.all,"]"))
ggsave("results/f6-did2-event-ch-all.png", dd.ch.plot)  
...
dd.ch.plot <- ggdid(event.study1) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.12, label=paste0("Overall ATT 95% CI \n[",low.ci1,", ",high.ci1,"]"))
ggsave("results/f6-did2-event-ch1.png", dd.ch.plot)  
...
dd.ch.plot <- ggdid(event.study2) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.12, label=paste0("Overall ATT 95% CI \n[",low.ci2,", ",high.ci2,"]"))
ggsave("results/f6-did2-event-ch2.png", dd.ch.plot)  
...
plot.es <- dwplot(event.study,
  ...
  annotate(geom="text", y=5.5, x=0.105, label=paste0("CH ATT: [",low.ci1,", ",high.ci1,"]"), hjust=0) +
  annotate(geom="text", y=5.5, x=0.09, label=paste0("NCH ATT: [",low.ci2,", ",high.ci2,"]"), hjust=0)
ggsave("results/f6-did2-event-ch.png", plot.es)
...
plot.nch <- dwplot(est.nch,
  ...
  scale_y_discrete(labels=c("time_p3"="t+3",
                            ...
                            "time_m2" = "t-2",
                            "time_m3" = "t-3"))
ggsave("results/f5-event-nch.png", plot.nch)
...
legend("bottomleft", pch=c(20,17,15), bty="n",
       legend=c("All Hospitals","CH Only","NCH Only"))
dev.copy(png,"results/f5-sa-nch.png")
dev.off()
...
## baseline results without adjustment
out.claim.all <- att_gt(yname="log_price",
                        gname="min_nch_entry",
                        idname="person_id",
                        tname="Year",
...
ggsave("results/f6-did-base-nch.png", plot.es)



## residualized results
out.claim.all <- att_gt(yname="resid",
                     gname="min_nch_entry",
                     idname="person_id",
                     tname="Year",
...
dd.nch.plot <- ggdid(event.study.all) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.12, label=paste0("Overall ATT 95% CI \n[",low.ci.all,", ",high.ci.all,"]"))
ggsave("results/f6-did-event-nch-all.png", dd.nch.plot)  
...
dd.nch.plot <- ggdid(event.study1) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.12, label=paste0("Overall ATT 95% CI \n[",low.ci1,", ",high.ci1,"]"))
ggsave("results/f6-did-event-nch1.png", dd.nch.plot)  
...
ggsave("results/f6-did-event-nch2.png", dd.nch.plot)  
...
plot.es <- dwplot(event.study,
  ...
  annotate(geom="text", y=5.5, x=0.11, label=paste0("CH ATT: [",low.ci1,", ",high.ci1,"]"), hjust=0) +
  annotate(geom="text", y=5.5, x=0.10, label=paste0("NCH ATT: [",low.ci2,", ",high.ci2,"]"), hjust=0)
ggsave("results/f6-did-event-nch.png", plot.es)



## residualized results (only allowing for procedure and place FEs)
out.claim.all <- att_gt(yname="resid2",
                        gname="min_nch_entry",
                        idname="person_id",
                        tname="Year",
...
dd.nch.plot <- ggdid(event.study.all) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.12, label=paste0("Overall ATT 95% CI \n[",low.ci.all,", ",high.ci.all,"]"))
ggsave("results/f6-did2-event-nch-all.png", dd.nch.plot)  
...
dd.nch.plot <- ggdid(event.study1) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.12, label=paste0("Overall ATT 95% CI \n[",low.ci1,", ",high.ci1,"]"))
ggsave("results/f6-did2-event-nch1.png", dd.nch.plot)  
...
dd.nch.plot <- ggdid(event.study2) + geom_hline(yintercept=0, linetype=2) +
  ...
  scale_color_manual(values=c("Black","Black")) +
  annotate(geom="text", x=2.5, y=0.10, label=paste0("Overall ATT 95% CI \n[",low.ci2,", ",high.ci2,"]"))
ggsave("results/f6-did2-event-nch2.png", dd.nch.plot)  
...
plot.es <- dwplot(event.study,
  ...
  annotate(geom="text", y=5.5, x=0.11, label=paste0("CH ATT: [",low.ci1,", ",high.ci1,"]"), hjust=0) +
  annotate(geom="text", y=5.5, x=0.10, label=paste0("NCH ATT: [",low.ci2,", ",high.ci2,"]"), hjust=0)
ggsave("results/f6-did2-event-nch.png", plot.es)
